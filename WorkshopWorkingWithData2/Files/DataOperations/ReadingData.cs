...
[assembly: InternalsVisibleTo("WorkingWithDataUnitTest")]
namespace WorkshopWorkingWithData.Files.DataOperations
{
	...
	internal sealed class ReadingData
	{
		...
		internal Tuple<DataTable, string> GetAllContactsCustomQuery()
		{
			...
			else if (UserConnection.DBEngine.DBEngineType == DBEngineType.MSSql)
			{
				custom = new CustomQuery(UserConnection,
		...
		internal Tuple<DataTable, string> GetAllDataReverseJoinEsq()
		{
			var esqResult = new EntitySchemaQuery(
				UserConnection.EntitySchemaManager, "Contact");
		...
		internal Tuple<DataTable, string> GetContactsWithMinutesSelect(Guid ContactId)
		{
			Select select = new Select(UserConnection)
				...
				.From("Activity")
				.Where("Activity", "OwnerId").IsEqual(Column.Parameter(ContactId))
