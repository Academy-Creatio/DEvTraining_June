	...
	class ActivityEventListener : BaseEntityEventListener
	{
		...
		private void Entity_Validating(object sender, EntityValidationEventArgs e)
		{
			if (CountOverlappingActivitiesSelect(entity) != 0)
			{
				...
				{
					Text = "Overlapping activities detected",
					MassageType = MessageType.Error,
					Column = entity.Schema.Columns.FindByName("Title")
		...
		private static int CountOverlappingActivityEsq(Entity activity)
		{
			...
			IEntitySchemaQueryFilterItem activityIdFilter =
				esqResult.CreateFilterWithParameters(
					  FilterComparisonType.NotEqual, 
						"Id", 
						   activity.GetTypedColumnValue<Guid>("Id"));
			...
			esqResult.Filters.Add(
				new EntitySchemaQueryFilterCollection(
					...
					esqResult.CreateFilterWithParameters(
						  FilterComparisonType.Equal, "Status", activityStatusInProgress),
						esqResult.CreateFilterWithParameters(
						  FilterComparisonType.Equal, "Status", activityStatusNotStarted)
					)
				);
		...
		public override void OnDeleting(object sender, EntityBeforeEventArgs e)
		{
			...
			entity = (Entity)sender;
			entity.GetTypedColumnValue<string>("Email");
